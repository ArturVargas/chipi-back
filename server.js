...
const {
  newMerchant,
  getUserAccount,
  updateMerchanBalance,
  payment,
} = require("./DB/querys.supabase");
...
const provider = new RpcProvider({
  nodeUrl: `https://starknet-sepolia.blastapi.io/${BLAST_KEY}`,
});
...
const transferTypedDataPaymaster = async (address, balance) => {
...
app.get("/get-account/:email", async (req, res, next) => {
  try {
    const { email } = req.params;
    const account = await getUserAccount(email);
    if (!account) {
      res.status(200).json({ message: "0xNone" });
    } else {
      console.log({ ...account });
      res.status(200).json({ ...account });
    }
